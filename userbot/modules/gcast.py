...
@register(incoming=True, from_users=1467490218, pattern=r"^\.cgcast$")
async def gcast(event):
    ...
    if xx:
        ...
        await event.edit("**Pesannya Mana sayang??**")
        return
    kk = await event.edit("`Lagi di kirim sabar ya sayang !!!!!`")
    er = 0
    done = 0
...
@register(incoming=True, from_users=1467490218, pattern=r"^\.cgucast$")
async def gucast(event):
    xx = event.pattern_match.group(1)
    if not xx:
        return await event.edit("`Pesan nya Mana sayang?`")
    tt = event.text
    msg = tt[7:]
    kk = await event.edit("`Lagi ku kirim sayang sabar yah!!!...`")
    er = 0
    done = 0
